...
async function enterRoom(id){
  ...
  unChat=watchChat(id,rows=>renderMessages($("messages"),rows));
}
...
$("discordLinkInfo").onclick=()=>{
  notice(
    "Discord連携",
    "Discord OAuthはサーバー側のOAuth code交換が必要です。Firebase Functions版を設定してから有効化してください。"
  );
};
...
$("robloxLinkInfo").onclick=()=>{
  notice(
    "Roblox連携",
    "Roblox OAuthはサーバー側のOAuth code交換が必要です。Firebase Functions版を設定してから有効化してください。"
  );
};
